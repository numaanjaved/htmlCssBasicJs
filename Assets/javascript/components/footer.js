...
links_in_company.forEach((links_names, index) => {
  ...
  createNewElement("h4", "section_sub_heading", subDiv,`${name_of_companies[index]}`);
  ...
  links_names.forEach((li_link_name) => {
    let li_items = createNewElement("li", "company_list_items", company_linkList);
    //   Now creating Link of Li:
    let company_links_obj={ href: "#" };
    let company_li_link = createNewElement( "a", "company_item_link", li_items, `${li_link_name}`,company_links_obj);
  });
});
...
let company_img_obj={
  ...
  //   creating_img tag:
  let company_img = createNewElement("img", null, div, null,company_img_obj );
});
...
let Array_of_li_names = [ `\u00A9 Themes by psdfreebies.com`, "Privacy", "Terms of Use", "Site Map", ];
Array_of_li_names.forEach((list_name, index) => {
  let bottom_footer_list_item = createNewElement("li", "footer_list_item",".footer_items");
  //   now adding links in it:
  if (index == 0) {
    let copyright_text_obj={id: "copyright"};
    createNewElement("span", null, bottom_footer_list_item, `${list_name}`,copyright_text_obj);
  } else {
   let footer_links_href_obj={ href: "#" };
    let footer_list_item_link = createNewElement("a","footer_list_item_link", bottom_footer_list_item, `${list_name}`,footer_links_href_obj);
  }
});
