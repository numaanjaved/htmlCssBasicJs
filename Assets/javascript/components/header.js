let createNewElement = ( newElement, elementClass = null, elementContainer, elementContent = null,attributes = {}) => {
  let htmlElement = document.createElement(newElement);
  if (elementClass) {
    if (Array.isArray(elementClass)) {
      elementClass.forEach((cls) => { htmlElement.classList.add(cls);});
    } else { htmlElement.classList.add(elementClass);}
  }
...
let rex_logo_img_obj= { src: "./Assets/images/BlueRex_Logo.webp", alt: "RexLogo Icon" }
createNewElement("img", "rexlogo", ".logo", null,rex_logo_img_obj );
...
array_of_nav_links.forEach((li_items) => {
  ...
  let navbar_links_href_obj = {href: `${li_items.href}`};
  createNewElement("a", "li_links", li, `${li_items.text}`, navbar_links_href_obj);
});
